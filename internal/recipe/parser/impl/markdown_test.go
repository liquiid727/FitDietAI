...
func TestParseBySize(t *testing.T) {
    ...
    for i := 0; i < 100; i++ { sb.WriteString("行内容\n") }
    ...
    if len(chunks) == 0 { t.Fatalf("expected non-empty chunks") }
    if !strings.Contains(chunks[0].Source, ".md") { t.Fatalf("source missing path") }
}
...
func BenchmarkParseByHeader(b *testing.B) {
    dir := b.TempDir()
    var sb strings.Builder
    for i := 0; i < 500; i++ { sb.WriteString("# H\n内容\n") }
